<template>
	<div id="a_address">
		<div class="a_header">
			<mu-icon value="navigate_before" @click="$router.go(-1)" /> 我的地址
		</div>
		<div class="ad_main">
			<ul class="details">
				<li>
					...
					if(!reg.test($phone.val())) {
						...
						if(this.area == '请选择地区') {
							...
							if(!reg.test($address.val())) {
								...
								//添加成功，准备跳转
								//将信息添加到数据库
								//axios.get('http://10.3.136.62:88/getorder',params{{phone:this.phone},{name:$('#id').val()},{relation:$('#phone').val()},{localarea:this.area},{address:$('#address').val()}}).then(res)=>{
								//console.log(res);
								//}
